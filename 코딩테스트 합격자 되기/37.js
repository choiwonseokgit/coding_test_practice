...
function solution(n, costs) {
  // 6. 비용을 기준으로 간선을 오름차순 정렬
  costs.sort((a, b) => a[2] - b[2]);

  // 7. 각 노드의 부모를 추적하는 parent 배열 생성
  const parent = Array.from({ length: n }, (_, i) => i);

  // 8. 각 노드의 트리의 랭크를 추적하는 rank 배열 생성
  ...
  for (const edge of costs) {
    if (edge === n - 1) {
      // 9. n - 1개의 간선이 포함된 경우 중단(최소 신장 트리의 속성)
      break;
    }

    // 10. 현재 간선의 두 노드가 속한 집합의 루트 노드 찾기
    const x = find(parent, edge[0]);
    const y = find(parent, edge[1]);

    if (x !== y) {
      // 11. 두 노드가 서로 다른 집합에 속하는 경우, 집합 합치기
