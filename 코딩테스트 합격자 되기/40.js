...
function solution(graph, start) {
  ...
  while (queue.size() > 0) {
    ...
    // 6. 만약 현재 노드의 거리 값이 큐에서 가져온 거리 값보다 크면, 해당 노드는 이미 처리한 것 이므로 무시
    if (distances[currentNode] < currentDistance) {
      continue;
    }

    // 7. 현재 노드와 인접한 노드들의 거리 값을 계산하여 업데이트
    for (const adjacentMode in graph[currentNode]) {
      const weight = graph[currentNode][adjacentMode];
      const distance = currentDistance + weight;

      // 8. 현재 계산한 거리 값이 기존 거리 값보다 작으면 최소 비용 및 최단 경로 업데이트
      if (distance < distances[adjacentMode]) {
        distances[adjacentMode] = distance; // 최소 비용 업데이트
        // 최단 경로 업데이트
        paths[adjacentMode] = [...paths[currentNode], adjacentMode];

        // 9. 최소 경로가 갱신된 노드를 비용과 함께 큐에 푸시
        queue.push([distance, adjacentMode]);
      }
    }
  }
